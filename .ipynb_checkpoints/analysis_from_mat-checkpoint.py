...
def plot_poincare_2d(poincare_coords, title="2D Hyperbolic Embedding (Poincare Disk)"):
    """
    ...
    ax.scatter(poincare_coords[:, 0], poincare_coords[:, 1], c='b', s=20)
    ax.set_title(title)
    ax.set_aspect('equal', 'box')
    plt.show()

def plot_poincare_3d(poincare_coords):
    """
    Creates a 3D visualization of points in the Poincare ball.
    ...
    print("Generating 3D Poincare ball visualization...")
    ...
    ax.scatter(poincare_coords[:, 0], poincare_coords[:, 1], poincare_coords[:, 2], c='b', s=20)
...
def visualize_embedding(fit_results):
    ...
    if dim == 2:
        plot_poincare_2d(coords)
    elif dim == 3:
        plot_poincare_3d(coords)
    elif dim > 3:
        ...
        plot_poincare_2d(projected_coords, title=title)
...
def run_embedding(dmat, embedding_dim, verbose=False):
    """
...
def run_euclidean_embedding(dmat, embedding_dim):
    ...
    normalized_dmat = 2.0 * dmat / np.max(dmat)
    ...
    # Calculate the pairwise distances in the new embedding
    embedded_dmat = pairwise_distances(coords, metric='euclidean')

    return {'dmat': normalized_dmat, 'emb_mat': embedded_dmat, 'coords': coords}
